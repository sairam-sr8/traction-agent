...
with st.form("traction_form"):
    ...
    with col1:
        filter_device = st.text_input("Filter: Device Type (optional)", "")
    with col2:
        filter_region = st.text_input("Filter: Region (optional)", "")
    with col3:
        filter_campaign = st.text_input("Filter: Campaign (optional)", "")
...
if submitted and events:
    # Build filters dict
    filters = {}
    if filter_device.strip():
        filters["device_type"] = filter_device.strip()
    if filter_region.strip():
        filters["region"] = filter_region.strip()
    if filter_campaign.strip():
        filters["campaign"] = filter_campaign.strip()
    with st.spinner("Analyzing traction metrics..."):
        result = analyze_traction(startup_id, events, filters=filters if filters else None)
